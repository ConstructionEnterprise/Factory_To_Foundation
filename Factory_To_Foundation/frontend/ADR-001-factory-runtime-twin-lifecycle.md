...
**Status:** **Accepted and IMPLEMENTED — B1 (dedicated Factory Runtime service).** Decision recorded 2026-08-11 (see §8). **Cutover executed and fully verified 2026-08-11** (migration plan §7 all steps; §9 Tests A and B/B1/B2 all PASS — process-kill/recovery lifecycle, 30-minute FF-idle observation, and natural auth-token-expiry recovery all confirmed live): `factory-runtime/` is the real, sole Twin owner; `twin-bridge/` stopped and renamed to `twin-bridge--DO-NOT-USE/` (kept intact, unmodified, as the rollback path only — not an active runtime). Two real bugs found and fixed during verification (a manifest-response array-spread regression, and the Twin hooks' own bypass of FF's existing auth-refresh mechanism) — see the migration plan's own status block for both. **Not yet deployed to production** — everything above is verified locally only. **See the migration plan's own status line for the complete, current record — including the pre-existing, migration-independent `robots.A2`×`roller` CE e-stop this cutover's own testing repeatedly and deterministically reproduced (also documented in CE's own `Chappell_Robotics/CLAUDE.md`).**
**Date:** 2026-08-11
