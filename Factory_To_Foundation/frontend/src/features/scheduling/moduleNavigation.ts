...
/** Real path for a real module id, or undefined if the id isn't one of the real 12 (or the task has no owner yet). */
export function pathForModuleId(moduleId: string | null): string | undefined {
  if (!moduleId) return undefined;
  return MODULE_ID_TO_PATH[moduleId];
}

/** Real module id/label pairs for a task-creation form's owner picker — same 12 real ids the RBAC seed uses, not invented. */
