...
- **`EntityCanvas` / `EntityNode`** (`framework/viewport`) — the generic "nodes positioned in world-space, connected by optional edges, click to select" renderer. Originally bespoke per-feature (`GraphNode`/`GraphCanvas`, `FactoryNode`/`FactoryCanvas`); promoted to `framework/` once Robotics/Logistics/Construction/Assets all needed the same shape at once. Genealogy, Factory, and (as of the Phase 1 function-block work, §6.4) Scheduling all use their own bespoke node components (`GraphNode`, `FactoryNode`, `FunctionBlockNode`/`FunctionBlockCanvas`) because each has feature-specific visual treatment that doesn't fit the generic component's props (Genealogy: tier-colored borders; Factory: a left-edge status strip; Scheduling: named ports on the block edges, wires that must land on exact pin coordinates rather than box-center-to-box-center) — not a gap, a deliberate non-migration in each case. Factory's node no longer has a `tier` concept at all — that field (and its `line`/`cell`/`machine`/`station` fixture values) was removed along with `factoryData.ts`; every live node gets the same visual treatment, distinguished only by `status` (`running`/`idle`/`down`/`unknown`). `FactoryCanvas` takes its `nodes` array as a prop (rather than importing data internally) — `FactoryLayout` passes manifest-driven-live nodes, or an empty array when the twin bridge isn't reachable (no fixture left to fall back to).
...
| **Scheduling** | `scheduleData.ts` — **the real 5-type taxonomy the user specified**: Inbound Material → Material Arrival → Sub-Assembly/Module Production → Storage & Logistics → Construction Schedule, now modeled as real **IEC 61131-3-style function blocks** — named, typed ports and port-to-port wires, not generic node-to-node connectors | **Phase 1 function blocks, structural only — see §6.4.** No KPIs (nothing real to summarize). Inspector shows real Inputs/Outputs port lists alongside the existing honest "No live schedule data yet — owned by [Logistics/Factory/Construction]" note. `inbound-material` has no owning feature yet — flagged as an open gap. Deliberately NOT migrated to `BrowseList`'s `children` mode — it's a linear pipeline, not a containment hierarchy. |
...
10. **Scheduling's function blocks (§6.4) are Phase 1 only — no execution engine.** An actual interpreter that evaluates the wired graph on a cycle and propagates real values through the ports was explicitly scoped out and deferred, not forgotten — and it's a real open question whether that engine belongs in the frontend at all, versus server-side closer to the twin's own tick loop. Don't start building it without confirming scope first, the same way Phase 1's scope was pinned down before any code was written.
...
### 6.4 Scheduling — Function Blocks, Phase 1 (structural only, no execution)

**What "function block" means here, precisely:** IEC 61131-3's real term — a reusable, *stateful* unit (unlike a stateless "Function," an FB instance retains internal state between calls) with named, typed inputs/outputs, wired to other blocks' ports, evaluated on a repeating scan cycle. Scheduling's 5 real stages (already user-specified, not invented) are now modeled with real named ports and port-to-port wires instead of the generic node-to-node connector lines `EntityCanvas` drew before. **Deliberately Phase 1 only — no execution.** No tick/evaluation loop, no value propagation through the wires, nothing runs. An actual interpreter (Phase 2, if it ever happens) is explicitly out of scope for this pass and arguably isn't even a frontend concern — it would want to live server-side, closer to the twin's own `advance()` tick loop than to React.

**The real ports** (all `PortType = "BOOL"` for Phase 1 — one minimal type, deliberately not inventing a richer payload shape without a real reason to yet):

```
Inbound Material                    →  PO Placed
Material Arrival                    →  Material Received     (in: PO Placed)
Sub-Assembly & Module Production    →  Module Complete        (in: Material Received)
Storage & Logistics                 →  Ready for Install      (in: Module Complete)
Construction Schedule                                          (in: Ready for Install)
```

4 real wires, one per consecutive pair, each connecting a specific output port id to a specific input port id — not a generic line between two boxes.

**A real, concrete future wiring point, deliberately not connected yet:** the digital twin's `state.json` already has a real `module_done: boolean` field — conceptually the same signal as `Module Complete`. Noted as a comment in `scheduleData.ts`, not wired — connecting it would be execution (reading a live value and driving a port), out of scope for Phase 1.

**What was built:** `scheduleData.ts` gained `FunctionBlockPort`/`FunctionBlockWire` types and `getPortOffsetY(nodeHeight, index, count)` — a single geometry function shared by the node (drawing its own pins) and the canvas (drawing wires that must land exactly on them), so the two can't drift apart; distributes ports evenly rather than assuming "exactly one per side" even though that's true of every block today. New bespoke `FunctionBlockNode.tsx`/`.css` (labeled pins on the left/right edges — a real attachment point, not just text) and `FunctionBlockCanvas.tsx`/`.css` (reuses the shared `useCamera` engine; wires computed from real pin coordinates). `ScheduleLayout`/`ScheduleBrowse`/`ScheduleInspector` all repointed; `SchedulePayload` gained `inputs`/`outputs` port lists, shown as new Inspector rows (`"None"` where a block genuinely has none — Inbound Material has no inputs, Construction Schedule has no outputs).

**Verified:** `tsc -b --force` clean, every changed module confirmed Vite-transformable. **Not confirmed:** an in-browser screenshot — same no-browser-automation-tool gap as §6.3's Factory verification.
